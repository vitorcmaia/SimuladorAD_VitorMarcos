...
public class FullNetworkTest {
	...
	public void PacksFromRouterIntoSameRx() {
		...
		// Deve ser vazia a sequ�ncia, pois todos os pacotes chegaram, na ordem certa.
		assertEquals(sack.getSequences().size(), 0);
	}
	
	@Test
	// Crio 5 pacotes e envio 3, "esque�o" os pacotes 2 e 4.
	// Depois, envio.
	public void MissingPacksArrivingLateIntoSameRx() {
		...
		// Deve ser vazia a sequ�ncia, pois todos os pacotes chegaram, mesmo fora de ordem.
		assertEquals(sack.getSequences().size(), 0);
	}
	
	@Test
	// Entrego todos os pacotes, numa ordem aleat�ria.
	...
	public void PacksIntoManyRxs() {
		...
		s.getRouter().sendPackToRx(0.0);
		s.getRouter().sendPackToRx(0.0);
		// Sei que os dois �ltimos pacotes s�o de origens diferentes :D
